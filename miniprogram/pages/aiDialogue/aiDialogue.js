...
Page({
  ...
  recordDialogueAnswer(answer, bigStageIdx, subTaskIdx) {
    ...
    const exists = this.data.dialogueRecord.some(
      r => r.bigStageIndex === record.bigStageIndex && r.subTaskIndex === record.subTaskIndex
    );
    if (!exists) {
      this.setData({
  ...
  startNewTask() {
    ...
    this.setData({
      messages: [...this.data.messages, {
        speaker: 'ai',
        content: `您好！我是AI健康助手，${firstStage.description}\n\n` + firstTask.detail,
 ...
 async callAIModel(userMessage) {
  ...
  onShow() {
    ...
    if (!this.data.messages || this.data.messages.length === 0) {
      this.addMessage('ai', '我是"医伴"小助手，可以向我咨询有关医疗或医院相关问题，点击右下角按钮可进入智能导诊模式', [
        '医院科室介绍',
        '挂号流程说明',
        '医院导航',
      ]);
